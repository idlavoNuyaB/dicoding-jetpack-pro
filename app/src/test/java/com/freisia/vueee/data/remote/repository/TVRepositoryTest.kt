...
class TVRepositoryTest {
    ...
    fun getDetail() = runBlocking {
        ...
        assertEquals("The Mandalorian", response.body()?.name)
    }
    ...
    @Test
    fun getOnAirList() = runBlocking{
        val response = repository.getOnAirList(1)
        assertEquals(20, response.body()?.result?.size)
    }

    @Test
    fun getTopRatedList() = runBlocking{
        val response = repository.getTopRatedList(1)
        assertEquals(20, response.body()?.result?.size)
    }
